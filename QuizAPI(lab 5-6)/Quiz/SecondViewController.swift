...
class SecondViewController: UIViewController, GoBackDelegate {
    ...
    func initData(){
        ...
        APIManager.goToAPI {
            (data) -> Void in
            do{
                let allQuestion = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.allowFragments) as! [String : AnyObject]
                if let arrJSON = allQuestion["question"] as? [[String: Any]] {
                    for item in arrJSON{
                        self.newList.append(Question(que: item["que"] as! String, cur: "", cor: item["cor"] as! String, corB: false, varA: item["varA"] as! [String]))
                    }
                }
            }
            catch{
                self.newList = Quiz.sharedInstance.generate(arrQue: self.arrQuestion)
            }
            defer{
